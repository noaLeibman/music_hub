...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.UserInfo(email=user.email, hashed_password=hashed_password, full_name=user.full_name)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def check_username_password(db: Session, user: schemas.UserCreate):
    ...
    return bcrypt.checkpw(user.hashed_password.encode('utf8'), db_user_info.hashed_password)
